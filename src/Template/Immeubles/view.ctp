 ...
 * @var \App\Model\Entity\Immeuble $immeuble
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Immeuble'), ['action' => 'edit', $immeuble->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Immeuble'), ['action' => 'delete', $immeuble->id], ['confirm' => __('Are you sure you want to delete # {0}?', $immeuble->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Immeubles'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Immeuble'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Employes'), ['controller' => 'Employes', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Employe'), ['controller' => 'Employes', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="immeubles view large-9 medium-8 columns content">
    <h3><?= h($immeuble->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Address') ?></th>
            <td><?= h($immeuble->address) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($immeuble->id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Employes') ?></h4>
        <?php if (!empty($immeuble->employes)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($immeuble->employes as $employes): ?>
            <tr>
                <td><?= h($employes->id) ?></td>
